...
public class SetleaderCommand implements SubCommand {
    ...
    public void perform(Player player, String[] args, HCFPlugin plugin) throws NotInFaction {
        ...
        if(!f.getLeader().equals(player.getUniqueId())) {player.sendMessage(ConfigManager.MUST_BE_LEADER);return;}
        Player target = Bukkit.getPlayer(args[0]);
        if(target == null) {player.sendMessage(ConfigManager.PLAYER_NOT_FOUND);return;}
        if(!f.getMembers().contains(target.getUniqueId())){player.sendMessage(ChatColor.RED + "Player is not in your faction");return;}
